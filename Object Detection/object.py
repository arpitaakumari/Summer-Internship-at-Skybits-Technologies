# -*- coding: utf-8 -*-
"""
Created on Fri Jul 24 16:12:00 2020
...
#loading video
cap=cv2.VideoCapture("F:/objectdetection/object.mp4")
...
while True:
    ...
    #out.write(frame)
    if height>1280 & width>720:
        cv2.imshow("Object_Detection",cv2.resize(frame,(1280,720)))
    else:
        cv2.imshow("Object_Detection",frame)   
    #key = cv2.waitKey(1)#wait 1ms the loop will start again and we will process the next frame
    if writer is None:
        fourcc = cv2.VideoWriter_fourcc(*"MJPG")
        writer = cv2.VideoWriter("output_object.avi", fourcc, 30,
